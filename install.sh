...
# ─── Step 5: 全量 Agent Adapter ───
echo ""
echo "⑤ 一键配置全部支持的 Agent..."
...
if [ -n "$ADAPTER_INSTALL_FAILED" ]; then
  echo "   adapter_install_partial_failures:$ADAPTER_INSTALL_FAILED"
  echo "   提示：这些通常是本机没有对应客户端、客户端配置损坏，或客户端 CLI 自身失败。"
  echo "   修复后可单独运行: memory adapter install <adapter>"
  if [ "$AMH_STRICT_ADAPTER_INSTALL" = "1" ]; then
    exit 1
  fi
else
  echo "   ✅ 所有可安装 adapter 已尝试配置完成"
fi

ADAPTER_STATUS_LABEL="✅ 安装完成！"
ADAPTER_MODULE_COPY="全部支持的 adapter 已配置: $AMH_INSTALL_ADAPTERS"
if [ -n "$ADAPTER_INSTALL_FAILED" ]; then
  ADAPTER_STATUS_LABEL="⚠️ 安装完成，但部分 Agent Adapter 配置失败"
  ADAPTER_MODULE_COPY="已尝试全部支持的 adapter；失败:$ADAPTER_INSTALL_FAILED"
fi

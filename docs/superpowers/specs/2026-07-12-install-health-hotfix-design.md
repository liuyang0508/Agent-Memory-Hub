...
clone、install 和 first-search 三个阶段统一通过同一个 sanitized environment helper 启动。除设置上述隔离路径外，helper 还必须清除会把 Python/pip 写入重定向到临时根目录外的宿主变量：`PIP_TARGET`、`PIP_PREFIX`、`PIP_ROOT`、`PIP_CONFIG_FILE`、`PYTHONHOME`、`VIRTUAL_ENV` 和 `UV_PROJECT_ENVIRONMENT`；`AGENT_MEMORY_HUB_HOME` 显式指向 `BENCH_ROOT/agent-memory-hub`。保留运行必需的 `PATH`、locale 和代理变量，避免 `env -i` 破坏真实安装条件。

移除当前真实 `HOME` 下 `/remember` 的备份与恢复逻辑。cleanup 只负责删除整个 `BENCH_ROOT`；`--keep` 时保留完整根目录用于审计。安装失败和首次搜索都必须使用隔离环境，避免某一阶段重新回落到宿主 HOME。

clone、install 和 first-search 三个阶段都必须显式检查退出码。任一阶段失败时，benchmark 输出该阶段的有界日志并返回非零，不能继续计算出 `PASS`。`INT` 和 `TERM` 使用显式 signal trap 转换成非零退出，统一由 `EXIT` trap 执行一次 cleanup。

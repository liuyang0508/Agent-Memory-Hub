...
**Architecture:** Quickstart 通过一个 shell sanitized-environment helper 统一运行 clone、install、search。Python 侧新增只读 `adapter_health` 服务负责 AMH footprint 识别和结构化诊断，CLI doctor 只负责展示、输出裁剪和最终退出码。
...
- [ ] **Step 3: 用完整 sanitized environment 重写 quickstart**

将 `benchmarks/quickstart-60s.sh` 重写为下面的实现。三个阶段必须统一走 `run_isolated`，阶段命令先写日志，再有界输出，不能依赖可能吞掉退出码的 pipeline：
...
2. repair table 的 detail 改用 `bounded_diagnostic_text(action.detail)`。
...
repair_table.add_row(
    adapter_health.bounded_diagnostic_text(action.name),
    f"[{style}]{status}[/{style}]",
    adapter_health.bounded_diagnostic_text(action.detail),
)
```

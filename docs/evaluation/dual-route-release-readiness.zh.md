...
最终独立运行使用真实 base worktree hook 与 candidate hook、同一隔离 brain、同一公开合成
payload、同一 Python 环境和离线 HashingEmbedder。由于 base/candidate 对外都是 adapter
`hookSpecificOutput` envelope，而 benchmark runner 校验内部四键协议，两边使用同一个
协议 normalizer：stdin 原样交给真实 hook；只在外层 event name、非空
`additionalContext`、`<agent_brain>` 边界和公开 sentinel 均正确时，转换成 benchmark
四键 `injected/included` 结果。normalizer 不改查询、不预热进程、不绕过真实 hook。

runner 参数固定为 30 measured samples、3 warmups、交错 old/new、期望 `injected`；5 秒只
是观测窗口，发布判定仍使用 candidate 单次 2000ms 和 p95 增量 150ms 的原门槛。最终
独立运行没有并发 hook 测试，且没有保存或输出原始 Prompt。

| command | samples | p50 | p95 | max | errors | timeouts |
|---|---:|---:|---:|---:|---:|---:|
| base hook | 30 | 3052.442ms | 3278.290ms | 3379.994ms | 0 | 0 |
| candidate hook | 30 | 1670.694ms | 1773.504ms | 1809.085ms | 0 | 0 |

candidate p95 相对 base 为 -1504.786ms，benchmark exit code 为 0，性能子门禁 PASS。

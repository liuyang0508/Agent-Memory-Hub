...
**生成时间**：2026-07-02T18:02:34.157559+00:00
**数据来源**：`memory benchmark system`
**结论**：PASS，cases=240，items=1314，top_k=10。
...
| 指标 | Token | 说明 |
| --- | --- | --- |
| 全文详情预算 | 218564 | 如果把命中 item 的详情全部放入上下文，benchmark 记录的 token 预算。 |
| ContextPack 注入 | 53725 | 经过 locator / overview / detail_uri 分层装载后的实际注入预算。 |
| 节省 Token | 164839 | full_tokens - packed_tokens，负数按 0 处理。 |

ContextPack 节省率：**75.42%**；压缩比：**0.246**。
...
当前矩阵读取的是 adapter capability 记录：total=16，ready=15，verified=12，runtime_observed=10。

| Agent | 状态 | 证据等级 | 运行观测 | 接入模式 | 阻塞项 |
| --- | --- | --- | --- | --- | --- |
| aider | ready | verified | no | file | - |
| aone_copilot | ready | verified | no | file | - |
| claude_code | ready | install-ready | yes | command, hook, mcp | evidence level is install-ready, not verified |

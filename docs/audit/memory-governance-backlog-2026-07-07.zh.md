...
- Lifecycle 治理已补只读 checklist 和 JSON 队列：`memory govern plan --category lifecycle --format markdown` 给人读，`--format json` 输出 `review_queue`，每条包含 `memory read ... --head 2000 --view detail`、`can_auto_apply=false` 和 supersede / archive 边界。

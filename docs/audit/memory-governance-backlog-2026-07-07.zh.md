...
- Lifecycle 治理已补只读 checklist：`memory govern plan --category lifecycle --format markdown` 会列出每条 stale signal / handoff 的 `memory read ... --head 2000 --view detail` 复核命令和 supersede / archive 边界。

...
|---|---|---|---|---|---|---|
| P0 | 记忆注入 | 长中文/中英混合任务会退化成单个泛词关键词 | 复现样本中原输出为 `agent` 或 `agent\|amh\|hopfield`；修复后输出 `多agent协作\|长期记忆\|上下文工程\|共享可信上下文...` | 召回偏题，容易注入微信公众号配图、封面图等无关记忆 | 已代码闭环：补 `query_signal` 单测和真实 hook 端到端回归 | 继续把真实长 prompt 样本沉淀到 query signal corpus |
...
- `tests/unit/test_query_signal.py::test_long_mixed_agent_article_prompt_keeps_cjk_domain_keyphrases` 已通过。
- `tests/unit/test_query_signal.py tests/unit/test_adapter_runtime_events.py` 已通过，82 passed。

...
| Web surface | 91 API/WS routes | Cockpit、data-flow、memory-lineage、adapter onboarding、candidate review、lifecycle governance、Headroom、ML gate 等已进入 Web 面。 |
...
- `memory api-docs` 已改为从 `web.app` 动态枚举，与实际 Web 91 routes 同步。
...
2. 直接给硬指标：28 MCP tools、91 Web routes、16 adapters、13 verified、2 install-ready、1 wip、1483 tests。
...
3. 持续用测试锁住 `memory api-docs`、README、架构图谱与实际 Web 91 routes 的一致性。

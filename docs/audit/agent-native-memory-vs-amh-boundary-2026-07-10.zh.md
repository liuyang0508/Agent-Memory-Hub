...
| Web Admin 不等于 Agent 通道 | `memory serve` 是人类治理面，默认只监听 `127.0.0.1`，CORS 默认只放 loopback；Agent 默认应通过 CLI / MCP / hooks / awareness 接入 | 看到 8765 后管可打开，就宣称某个 Agent 已能读写 AMH |

    ...
    def _awareness_block(self) -> str:
        return render_awareness_block(
            ...
            extra_guidance=(
                ...
                "Hook execution alone is not enough: if no <agent_brain> block appears, follow the brief_memory project-recovery and full-task search_memory guidance above before non-trivial work.",
    ...
    def _native_memory_bridge_content(self) -> str:
        return "\n".join(
            [
                ...
                "",
                "恢复项目全貌时调用 `brief_memory`；面向当前具体任务时，把完整任务描述交给 `search_memory`；只对真正需要的 1-3 条调用 `read_memory`。",
                "`brief_memory` 与 `search_memory` 互补，不是相互失败降级。",
                "`project` 是 hard filter：只在用户明确指定，或 cwd 到项目的映射可确定时设置；不得根据自然语言猜项目。",
                "普通检索使用 auto 的 locator/overview 候选，只对真正相关的 1-3 条调用 read_memory；显式 detail 只用于有意、少量的诊断读取。",
                "产生可复用事实、决策、信号、产物或交接时，通过 `write_memory` 写回 AMH。",

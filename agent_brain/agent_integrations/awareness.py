...
def render_awareness_block(
    *,
    ...
    lines.extend(
        ...
        "- use `brief_memory` to recover the overall project state;",
        "- use `search_memory` with the full task description for the current concrete task;",
        "- `brief_memory` and `search_memory` are complementary, not fallbacks;",
        "- treat `project` as a hard filter: set it only when the user explicitly names the project or the cwd mapping is certain, never from a natural-language guess;",
        "- ordinary `search_memory(..., verbosity=\"auto\")` returns only locator/overview candidates;",
        ...
        "If MCP tools are available, call the appropriate `brief_memory` or `search_memory` path before replying;",

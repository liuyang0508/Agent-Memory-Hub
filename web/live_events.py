...
_sse_subscribers: list[tuple[asyncio.Queue, str | None, bool]] = []
_ws_subscribers: list[tuple[WebSocket, str | None, bool]] = []
...
async def _ws_send_and_reap(entry: tuple, text: str) -> None:
...
def broadcast_live_event(
    ...
    *,
    ...
    for entry in _sse_subscribers:
        q, sub_tenant, sub_is_admin = entry
    ...
    try:
        ...
        loop = None
    for entry in list(_ws_subscribers):
        ws, sub_tenant, sub_is_admin = entry
        if not _event_visible_to(sub_tenant, sub_is_admin, tenant_id=tenant_id, admin_only=admin_only):
            continue
        if loop is not None:
            loop.create_task(_ws_send_and_reap(entry, msg_str))

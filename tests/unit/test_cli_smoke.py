...
def test_api_docs_endpoint_rows_are_split():
    ...
    assert any(method == "POST" and path == "/api/governance/signals/{item_id}/transition" for method, path, _desc in API_ENDPOINTS)
    assert any(method == "POST" and path == "/api/governance/signals/recover" for method, path, _desc in API_ENDPOINTS)
...
def test_api_docs_cli_uses_current_web_route_count():
    ...
    assert result.exit_code == 0, result.output
    assert "Total: 122 endpoints" in result.output

...
def test_adapter_install_unavailable_dependency_reports_not_configured(monkeypatch):
    ...
    result = runner.invoke(app, ["adapter", "install", "openclaw"])

    assert result.exit_code == 1
    assert "openclaw: not configured" in result.output
    assert "openclaw CLI not found on PATH" in result.output

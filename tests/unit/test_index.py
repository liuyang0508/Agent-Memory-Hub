...
def test_index_falls_back_to_python_vectors_when_sqlite_vec_load_fails(
    tmp_brain_dir: Path,
    monkeypatch,
):
    from agent_brain.platform.indexing import index as index_mod

    def _boom(_connection):
        raise AttributeError("loadable extensions unavailable")

    monkeypatch.setattr(index_mod.sqlite_vec, "load", _boom)

    idx = index_mod.HubIndex(db_path=tmp_brain_dir / "index.db", embedding_dim=4)
    assert idx.vector.uses_sqlite_vec is False
    row = idx.connection.execute(
        "SELECT sql FROM sqlite_master WHERE name = 'items_vec'"
    ).fetchone()
    assert row is not None
    assert "CREATE TABLE" in row[0].upper()

    item_a, body_a = _make_item("fallback-a", "Fallback A", ["alpha"])
    item_b, body_b = _make_item("fallback-b", "Fallback B", ["beta"])
    idx.upsert(item_a, body_a, embedding=[1.0, 0.0, 0.0, 0.0])
    idx.upsert(item_b, body_b, embedding=[0.0, 1.0, 0.0, 0.0])

    hits = idx.vector_search([1.0, 0.0, 0.0, 0.0], top_k=2)
    assert [hit.id for hit in hits] == [item_a.id, item_b.id]
